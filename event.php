...
<head>
	...
	<title>
		<?php 
			$titleEvent = $conn->query('SELECT `tittle` FROM `event` WHERE `idEvent` = '.$page.'');
			foreach ($titleEvent as $row) {
				echo $row['tittle'];
			}
		?>
	</title>
...
<body>
	...
	<main>
		...
		<div class="event-wrapper">
			<?php 
				$textEvent = $conn->query('SELECT `tittle`, `eventText`, `dateEvent` FROM `event` WHERE `idEvent` = '.$page.'');
				foreach ($textEvent as $row) {
					echo '
						<p class="tittle-event">'.$row['tittle'].'</p>
						<div class="content-event-all">
			...
			?>
				<div class="video-photo-event">
					<div class="photo-event">
						<swiper-container class="mySwiper" navigation="true">
							<?php 
								$photoEvent = $conn->query('SELECT `path` FROM `event_photos` WHERE `idEvent` = '.$page.'');
								foreach ($photoEvent as $row) {
									echo '
										<swiper-slide><img src="'.$row['path'].'" /></swiper-slide>
									';
								}
							?>
						 </swiper-container>
					...
					<div class="video-event">
						<?php 
							$videoEvent = $conn->query('SELECT `path` FROM `event_video` WHERE `idEvent` = '.$page.'');
							foreach ($videoEvent as $row) {
								echo $row['path'];
							}
						?>
					</div>
